function [alpha1,beta1,gamma1,delta1,Lambda1,Kappa1,lambdaFun,kappaFun,tau1,time_delay1,NPI_matrix] = fit_SEIQRDP(Q,R,D,Npop,E0,I0,C0,time,guess,NPIs,varargin)
...
%   C0: scalar [1x1]: Initial number of confined cases
...
p.addOptional('tolX',1e-6);  %  option for optimset, 1e-5
p.addOptional('tolFun',1e-6);  %  option for optimset, 1e-5
...
options=optimset('TolX',tolX,'TolFun',tolFun,...
    'MaxFunEvals',2000,'Display',Display);
...
ub = [1, 5, 1, 1, lambdaMax, kappaMax, 1, 10, 0, 0, 0, 0];
lb = [0, 0, 0, 0, lambdaMin, kappaMin, 0,  1, -4, -4, -4, -4];
% call Lsqcurvefit
[Coeff] = lsqcurvefit(@(para,t) modelFun1(para,t),guess,tTarget(:)',input,lb,ub,options);
...
NPI_matrix = [10^Coeff(13), 10^Coeff(14), 10^Coeff(15), 10^Coeff(16)];
...
%% nested functions
    ...
    function [output] = SEIQRDP_for_fitting(para,t0)
        ...
        if ~isempty(R)
            Y(5,1) = R(1);
            Y(1,1) = Npop-Q(1)-R(1)-D(1)-E0-I0-C0;
        else
            Y(1,1) = Npop-Q(1)-D(1)-E0-I0-C0;
        end
        Y(6,1) = D(1);
        Y(7,1) = C0;
        ...
        for ii=1:N-1
            ...
            if ii > time_delay
                if NPIs(ii-time_delay, 2) == 1
                    alpha = alpha + 10^npis(NPI_ind);
                    NPI_ind = NPI_ind + 1;
                elseif NPIs(ii-time_delay, 2) == 2
                    tau = tau + 10^npis(NPI_ind);
                    NPI_ind = NPI_ind + 1;
                end
            end
